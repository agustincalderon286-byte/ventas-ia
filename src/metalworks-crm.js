      ...
      try {
        if (!searchKingsWebhookConfigured()) {
          return respondError(
            res,
            503,
            "SearchKings webhook auth is not configured yet.",
          );
        }
        ...
        if (!externalLeadId) {
          return respondError(res, 400, "Missing SearchKings call id.");
        }
        ...
        await appendActivity({
          leadId: leadDoc._id,
          activityType:
            cleanText(parsedEvent?.activity?.activityType || "", 80) || "searchkings_call",
          title: cleanText(parsedEvent?.activity?.title || "", 160) || "Llamada de SearchKings",
          body:
            cleanText(parsedEvent?.activity?.body || "", 2000) ||
            "SearchKings call data received.",
          meta: {
